...
| M4 Extension | F-15·F-16·명명 — [`features_extension.md`](features_extension.md) |
...
| **권장** | F-15 | 가중치 감성 분석 | 키워드 가중 합 비교; 혼합 텍스트; [`features_extension.md`](features_extension.md) §2 |
| **권장** | F-16 | FileHandler CSV 저장 | `saveFeedbacksCsv` / `saveAnalysisCsv`; analyze 후 export |
...
| **권장** | F-08 | KeywordRuleRepository (Port) | `FileKeywordRuleRepository` + `POST /keywords/register` — **구현 완료** |
...
| **선택** | F-11 | Trend 시각화 | index 트렌드 테이블 — **구현 완료** |
| **선택** | F-12 | File DB 키워드 관리 | `data/feedback_rules.json` — **구현 완료** |
| **선택** | F-13 | JSON API 응답 | `/api/session`, `/api/analyze`, `/api/filter` — **구현 완료** |
| **선택** | F-14 | PageLogSink level 외부화 | `POST /settings/log-levels` — **구현 완료** |
...
- [x] **INV-SENT-002:** 동일 세션에서 Analyze 직후 감정 건수 = Filter(`전체`,`전체`) 재집계 감정 건수 — TC-B-04 (2026-05-22 GREEN)
- [x] **INV-SENT-003:** Filter(`중립`,`전체`) 결과에 중립 라벨만 존재; 긍·부 샘플 제외 — TC-B-05
- [x] **INV-INPUT-001:** POST `/analyze` with `"   "` → 건수 불변 — TC-A-02, TC-B-07
- [x] **INV-SESSION-001:** 깨진/빈 CSV 업로드 → `error` + 기존 피드백 원문·건수 불변 — TC-A-05, TC-B-08
- [x] **INV-CSV-OUT-003:** Filter 후 download CSV 행·순서 = 필터 목록 — TC-A-04, TC-B-09, GM-TC-05
- [x] **INV-COUNT-002:** 긍정+중립+부정 = 전체 피드백 수 — TC-B-01, TC-B-06
- [x] **INV-EMPTY-001:** 0건 시 filter·download → warning, 집계/파일 미제공 — TC-A-03, GM-TC-04
- [x] **INV-TEXT-001:** analyze·export·HTML에 입력 원문(줄바꿈·장문) 보존 — TC-A-06, TC-B-10
...
| 1.2 | 2026-05-22 | REFACTOR: GH-01 추적 [`doc/gherkin_gh01.md`](gherkin_gh01.md), RR-1 **(B)** `화가` |
| 1.3 | 2026-05-22 | M4: F-15 가중치 감성, F-16 FileHandler, C++ 명명 — [`features_extension.md`](features_extension.md) |
| 1.4 | 2026-05-22 | M3: F-08·F-11~14, ST-06 동적 키워드, JSON API, Trend, PageLogSink — §8 |
