...
2. **기능 회귀 없음:** REFACTOR 전후 **52 passed** 유지, PRD §7.1 핵심 INV·Golden 계약 충족.  
3. **이중 트랙 해소:** HTTP 요청이 GREEN 단계에서 검증한 **동일 entity/control** 을 사용한다.  
4. **문서·계약 정합:** RR-1 `(B) 화가`, Gherkin 추적본, README TO-DO M2 핵심 항목 반영 완료.  
5. **다음 단계:** M3(🟢), F-08 KeywordRule, M4 네이밍은 별도 마일스톤에서 RED부터 진행 권장.
